...
const resetGame = () => {
  ...
  document.querySelector(".info").innerHTML = "Participant 1, move forward!";
...
const check = () => {
  ...
  if (!isbackground) {
    ...
    if (isTie) {
      setTimeout(() => {
        if (confirm("Nobody Wins !! Play again ?")) {
          resetGame();
        }
      }, 2000);
    }
    ...
    if (!isGameOver && box_text.innerHTML === "") {
      ...
      if (!isbackground && !isGameOver) {
        if (turn === "❌") {
            ...
            document.querySelector(".info").innerHTML = "Participant 2, move forward!";
        }
    }
